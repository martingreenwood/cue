...
@php
    $siteCopy ??= app(\App\Domains\CMS\Services\PublicSiteCopyService::class)->current();
    $customerSession ??= app(\App\Domains\Ticketing\Contracts\TicketingProvider::class)->customerSession();
@endphp
...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="min-h-screen bg-[#f5f0e8] font-sans text-[#171511] antialiased">
        ...
        <x-ticketing.customer-session-bar :customer-session="$customerSession" :site-copy="$siteCopy" />
        ...
        <header class="border-b border-[#171511]/10 px-5 py-6 sm:px-8 lg:px-12">
            <div class="mx-auto flex max-w-7xl items-center justify-between gap-6">
                <a href="{{ route('home') }}" class="inline-flex min-h-11 min-w-11 items-center text-2xl font-semibold tracking-[-0.05em] focus-visible:rounded-sm focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-[#a4432e]">
                    Cue
                </a>
